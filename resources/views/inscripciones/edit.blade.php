...
@section('content')
    ...
    <form method="POST" action="{{ route('inscripciones.update', $inscripcion->id) }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="num_pago" id="num_pago" class="form-control" value="{{ $inscripcion->num_pago }}" required>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="id_estudiante" id="id_estudiante" class="form-control" required>
                @foreach($estudiantes as $estudiante)
                    <option value="{{ $estudiante->id }}" {{ $inscripcion->id_estudiante === $estudiante->id ? 'selected' : '' }}>{{ $estudiante->nombre }} {{ $estudiante->apellido }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="id_clase" id="id_clase" class="form-control" required>
                @foreach($clases as $clase)
                    <option value="{{ $clase->id }}" {{ $inscripcion->id_clase === $clase->id ? 'selected' : '' }}>{{ $clase->nombre }}</option>
                @endforeach
            </select>
        </div>
